...
from profile_detector import get_override, detect_profile, main
...
class TestMainCLI:
    ...
    def test_no_args_shows_usage(self):
        with patch.object(sys, 'argv', ['profile_detector.py']):
            with pytest.raises(SystemExit) as exc_info:
                main()
            assert exc_info.value.code == 1

    def test_override_command(self, capsys):
        with tempfile.NamedTemporaryFile(mode='w', suffix='.json', delete=False) as f:
            json.dump({"activeProfile": "python-pytest"}, f)
            f.flush()
            with patch.object(sys, 'argv', ['profile_detector.py', 'override', f.name]):
                main()
            captured = capsys.readouterr()
            assert captured.out.strip() == "python-pytest"

    def test_override_command_no_output_for_empty(self, capsys):
        with tempfile.NamedTemporaryFile(mode='w', suffix='.json', delete=False) as f:
            json.dump({}, f)
            f.flush()
            with patch.object(sys, 'argv', ['profile_detector.py', 'override', f.name]):
        ...
        with tempfile.TemporaryDirectory() as project_dir:
            ...
            with tempfile.NamedTemporaryFile(mode='w', suffix='.json', delete=False) as f:
                json.dump({
                    "profiles": {
                        "typescript-npm": {
                            "detection": {"files": ["package.json"], "patterns": []}
                        }
                    }
                }, f)
                f.flush()
                with patch.object(sys, 'argv', ['profile_detector.py', 'detect', project_dir, f.name]):
                    main()
                captured = capsys.readouterr()
                assert captured.out.strip() == "typescript-npm"

    def test_override_missing_args(self):
        with patch.object(sys, 'argv', ['profile_detector.py', 'override']):
            with pytest.raises(SystemExit) as exc_info:
                main()
            assert exc_info.value.code == 1

    def test_detect_missing_args(self):
        with patch.object(sys, 'argv', ['profile_detector.py', 'detect', '/some/dir']):
            with pytest.raises(SystemExit) as exc_info:
                main()
            assert exc_info.value.code == 1

    def test_unknown_command(self):
        with patch.object(sys, 'argv', ['profile_detector.py', 'unknown']):
            with pytest.raises(SystemExit) as exc_info:
                main()
            assert exc_info.value.code == 1
