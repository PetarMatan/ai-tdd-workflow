...
class TestTDDLogger:
    """Tests for TDDLogger class."""

    def test_init_creates_log_directories(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            with patch.dict(os.environ, {"TDD_INSTALL_DIR": tmpdir}):
                logger = TDDLogger("test-session")
    ...
    def test_session_id_stored(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            with patch.dict(os.environ, {"TDD_INSTALL_DIR": tmpdir}):
                logger = TDDLogger("my-session-123")
                assert logger.session_id == "my-session-123"

    def test_get_timestamp_format(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            with patch.dict(os.environ, {"TDD_INSTALL_DIR": tmpdir}):
                logger = TDDLogger("test")
    ...
    def test_log_tdd(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            with patch.dict(os.environ, {"TDD_INSTALL_DIR": tmpdir}):
                logger = TDDLogger("test")
                logger.log_tdd("Phase transition to 2")
