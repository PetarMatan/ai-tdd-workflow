...
""")
    ...
    def test_derives_name_from_filename(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            filepath = os.path.join(tmpdir, "tdd-tester.md")
...
class TestMainCLI:
    ...
    def test_no_args_shows_usage(self):
        with patch.object(sys, 'argv', ['agent_parser.py']):
            with pytest.raises(SystemExit) as exc_info:
                main()
            assert exc_info.value.code == 1

    def test_get_phases_command(self, capsys):
        with tempfile.NamedTemporaryFile(mode='w', suffix='.md', delete=False) as f:
            f.write("""---
phases: [1, 2, 3]
---
""")
            f.flush()
            with patch.object(sys, 'argv', ['agent_parser.py', 'get-phases', f.name]):
                main()
            captured = capsys.readouterr()
            assert json.loads(captured.out) == [1, 2, 3]

    def test_get_name_command(self, capsys):
        with tempfile.NamedTemporaryFile(mode='w', suffix='.md', delete=False) as f:
            f.write("""---
name: My Agent
---
""")
            f.flush()
            with patch.object(sys, 'argv', ['agent_parser.py', 'get-name', f.name]):
                main()
            captured = capsys.readouterr()
            assert captured.out.strip() == "My Agent"

    def test_get_content_command(self, capsys):
        with tempfile.NamedTemporaryFile(mode='w', suffix='.md', delete=False) as f:
            f.write("""---
name: Test
---
...
""")
            f.flush()
            with patch.object(sys, 'argv', ['agent_parser.py', 'get-content', f.name]):
                main()
            captured = capsys.readouterr()
            assert "# Instructions" in captured.out

    def test_get_content_exits_on_empty(self):
        with tempfile.NamedTemporaryFile(mode='w', suffix='.md', delete=False) as f:
            f.write("")  # Empty file
            f.flush()
            with patch.object(sys, 'argv', ['agent_parser.py', 'get-content', f.name]):
    ...
    def test_agents_for_phase_command(self, capsys):
        with tempfile.TemporaryDirectory() as tmpdir:
            agent_path = os.path.join(tmpdir, "test.md")
            with open(agent_path, 'w') as f:
                f.write("""---
phases: [2]
---
""")
            with patch.object(sys, 'argv', ['agent_parser.py', 'agents-for-phase', tmpdir, '2']):
                main()
            captured = capsys.readouterr()
            assert "test.md" in captured.out

    def test_unknown_command(self):
        with patch.object(sys, 'argv', ['agent_parser.py', 'unknown']):
            with pytest.raises(SystemExit) as exc_info:
                main()
            assert exc_info.value.code == 1

    def test_missing_args_for_commands(self):
        commands = [
            ['agent_parser.py', 'get-phases'],
            ['agent_parser.py', 'get-name'],
            ['agent_parser.py', 'get-content'],
            ['agent_parser.py', 'list-agents'],
            ['agent_parser.py', 'agents-for-phase', '/some/dir'],
        ]
