#!/usr/bin/env python3
"""
Unit tests for tdd_supervisor/__main__.py - CLI entry point
"""
...
class TestHelpText:
    ...
    def test_parser_has_description(self):
        ...
        parser = argparse.ArgumentParser(
            description="TDD Supervisor - Orchestrate TDD workflow"
        )
        assert parser.description is not None
        assert "TDD" in parser.description
...
class TestEnvironmentVariables:
    ...
    def test_env_var_names_are_correct(self):
        ...
        for var in expected_vars:
            assert var.startswith("TDD_SUPERVISOR_")

    def test_supervisor_active_value(self):
        """TDD_SUPERVISOR_ACTIVE should be '1' when active."""
        expected_value = "1"
