...
from pattern_matcher import glob_to_regex, matches_pattern, matches_any, main
...
class TestMainCLI:
    ...
    def test_no_args_exits_with_usage(self):
        with patch.object(sys, 'argv', ['pattern_matcher.py']):
            with pytest.raises(SystemExit) as exc_info:
                main()
            assert exc_info.value.code == 2

    def test_match_command_missing_args(self):
        with patch.object(sys, 'argv', ['pattern_matcher.py', 'match']):
            with pytest.raises(SystemExit) as exc_info:
                main()
            assert exc_info.value.code == 2

    def test_match_command_exits_0_on_match(self):
        with patch.object(sys, 'argv', ['pattern_matcher.py', 'match', 'test.py', '*.py']):
            with pytest.raises(SystemExit) as exc_info:
                main()
            assert exc_info.value.code == 0

    def test_match_command_exits_1_on_no_match(self):
        with patch.object(sys, 'argv', ['pattern_matcher.py', 'match', 'test.py', '*.ts']):
            with pytest.raises(SystemExit) as exc_info:
                main()
            assert exc_info.value.code == 1

    def test_to_regex_command(self, capsys):
        with patch.object(sys, 'argv', ['pattern_matcher.py', 'to_regex', '*.py']):
            with pytest.raises(SystemExit) as exc_info:
                main()
            assert exc_info.value.code == 0
    ...
    def test_to_regex_missing_pattern(self):
        with patch.object(sys, 'argv', ['pattern_matcher.py', 'to_regex']):
            with pytest.raises(SystemExit) as exc_info:
                main()
            assert exc_info.value.code == 2

    def test_unknown_command(self):
        with patch.object(sys, 'argv', ['pattern_matcher.py', 'unknown']):
            with pytest.raises(SystemExit) as exc_info:
                main()
            assert exc_info.value.code == 2
