...
EOF
    cat > "$SOURCE_DIR/config/tdd-config.json" << 'EOF'
{
  "profiles": {}
}
EOF
    ...
    # Create skills
    echo '# TDD' > "$SOURCE_DIR/skills/tdd.md"
    echo '# Status' > "$SOURCE_DIR/skills/tdd-status.md"
    echo '# Reset' > "$SOURCE_DIR/skills/tdd-reset.md"
    echo '# Create' > "$SOURCE_DIR/skills/tdd-create-agent.md"
...
@test "update_settings adds TDD permissions" {
    cat > "$SETTINGS_FILE" << 'EOF'
{
  "permissions": {
    "allow": []
...
"
    ...
    # Verify PostToolUse tdd-auto-test hook structure
    run python3 -c "
import json
s = json.load(open('$SETTINGS_FILE'))
