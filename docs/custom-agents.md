...
Custom agents are one of the most powerful features of the TDD workflow. They let you inject domain expertise, coding standards, and specialized knowledge into each phase of development.

## What is an Agent?

An agent is a markdown file that gets loaded into Claude's context during specific TDD phases. Think of it as a system prompt that shapes how Claude thinks about your particular problem space.
...
Agents are stored in `~/.claude/tdd-workflow/agents/` and use YAML frontmatter to specify which phases they activate in.
...
Run `/tdd-create-agent` in Claude Code. Claude will walk you through:
...
6. **Phase Binding** - Which TDD phases should auto-load this agent

The agent file is created automatically at `~/.claude/tdd-workflow/agents/{name}.md`.

### Option 2: Manual Creation

Create a markdown file in `~/.claude/tdd-workflow/agents/` with this structure:
...
**List all agents:**
```bash
ls ~/.claude/tdd-workflow/agents/
```

**Edit an agent:**
```bash
$EDITOR ~/.claude/tdd-workflow/agents/your-agent.md
```

**Remove an agent:**
```bash
rm ~/.claude/tdd-workflow/agents/your-agent.md
```
