...
class TDDConfig:
    """Configuration manager for TDD workflow."""
    ...
    def __init__(self, project_dir: str = "."):
        ...
        self.config_file = os.environ.get(
            "TDD_CONFIG_FILE",
            os.path.join(install_dir, "config", "tdd-config.json")
        )
        self.override_file = os.environ.get(
            "TDD_OVERRIDE_FILE",
            str(Path.home() / ".claude" / "tdd-override.json")
        )
    ...
    def detect_profile(self) -> Optional[str]:
        ...
        default_profile = os.environ.get("TDD_DEFAULT_PROFILE")
        if default_profile:
