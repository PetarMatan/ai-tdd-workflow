#!/usr/bin/env python3
"""
TDD State Management - Unified State File

Consolidates all TDD workflow state into a single state.json file.
...
class CompletedPhases:
    """Tracks which TDD phases have been completed."""
...
class TDDState:
    """
    Unified TDD state management using a single state.json file.
    ...
    def __init__(
        ...
        if supervisor_markers_dir:
            ...
            if dir_name.startswith("tdd-supervisor-"):
                self.workflow_id = dir_name[len("tdd-supervisor-"):]
            else:
                self.workflow_id = os.environ.get("TDD_SUPERVISOR_WORKFLOW_ID", "")
        else:
            self._env_supervisor_mode = False
            if mode == "supervisor":
                self.workflow_id = workflow_id or self._generate_workflow_id()
                self.state_dir = self.base_dir / f"tdd-supervisor-{self.workflow_id}"
            else:
                self.workflow_id = workflow_id or ""
                self.state_dir = self.base_dir / f"tdd-{session_id}"
    ...
    def is_supervisor_mode(self) -> bool:
        ...
        if self._env_supervisor_mode:
            return True
        if os.environ.get("TDD_SUPERVISOR_ACTIVE") == "1":
            return True
        state = self._load_state()
    ...
    def cleanup_workflow_state(self) -> None:
        ...
        Used when TDD completes successfully.
        """
        state = self._load_state()
        state.active = False
        state.phase = 1
    ...
    def get_env_vars(self) -> Dict[str, str]:
        ...
        return {
            "TDD_SUPERVISOR_WORKFLOW_ID": workflow_id,
            "TDD_SUPERVISOR_MARKERS_DIR": str(self.state_dir),
