...
def main():
    ...
    logger.log_tdd(f"Phase 4: Running compile + test cycle for {hook.file_path}")
    print(f">>> TDD Phase 4 ({profile_name}): Running compile + test cycle...", file=sys.stderr)

    # Run compilation
    compile_exit_code, compile_output = run_command(compile_cmd, timeout=120)

    if compile_exit_code != 0:
        logger.log_build("FAILED", "TDD Phase 4 compilation failed")
        print(">>> TDD: Compilation failed", file=sys.stderr)

        context = format_phase4_compile_error(compile_output, hook.file_path, profile_name)

        approve_with_message(
            f"TDD Phase 4 ({profile_name}): Compilation failed, fix immediately",
            context
        )
        return

    logger.log_build("SUCCESS", "TDD Phase 4 compilation passed")
    print(">>> TDD: Compilation passed, running tests...", file=sys.stderr)

    # Run tests
    test_exit_code, test_output = run_command(test_cmd, timeout=300)

    if test_exit_code != 0:
        logger.log_tdd("Phase 4: Tests failed - continuing implementation")
        print(">>> TDD: Tests failed", file=sys.stderr)

        context = format_phase4_test_failure(test_output, hook.file_path, profile_name)

        approve_with_message(
            f"TDD Phase 4 ({profile_name}): Tests failing, continue implementing",
            context
        )
        return

    # Both passed!
    logger.log_tdd("Phase 4: All tests passing!")
    print(">>> TDD: All tests passing!", file=sys.stderr)
